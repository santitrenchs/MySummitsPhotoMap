...
function NewsletterForm({ locale }: { locale: PeakLocale }) {
      ...
      <div style={{ display: "flex", gap: 6 }}>
        <input
          ...
          style={{
            ...
            fontSize: 13, color: "#F0F4FF", outline: "none",
        ...
        <button
          ...
          style={{
            flexShrink: 0, background: "#2F7A5F", border: "none",
...
export function PeakFooter({ locale }: { locale: PeakLocale }) {
  const t = getLandingT(locale);
  ...
  return (
    <footer style={{ background: "#0D2538", borderTop: "1px solid rgba(255,255,255,0.06)", padding: "48px 0 32px" }}>
      <div style={{ width: "100%", maxWidth: 1160, margin: "0 auto", padding: "0 24px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr auto auto", gap: 48, marginBottom: 40 }} className="pk-footer-grid">

          {/* Brand + newsletter */}
          <div>
            <a href={homeHref} style={{ textDecoration: "none", display: "inline-block", marginBottom: 14 }}>
              <PeakadexLogo height={28} peakColor="#F0F4FF" adexColor="rgba(240,244,255,0.5)" iconBgColor="#0D2538" />
            </a>
            <p style={{ fontSize: 13, color: "rgba(240,244,255,0.38)", lineHeight: 1.6, maxWidth: 260, margin: "0 0 20px" }}>
              {t.footer_tagline1}<br />{t.footer_tagline2}
            </p>
            <p style={{ fontSize: 11, color: "rgba(240,244,255,0.22)", lineHeight: 1.5 }}>
              {t.footer_made}
            </p>
          ...
          {Object.entries(footerLinks).map(([title, links]) => (
            <div key={title}>
              <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", color: "rgba(240,244,255,0.35)", marginBottom: 16 }}>
                {title}
              </div>
              <ul style={{ listStyle: "none", margin: 0, padding: 0, display: "flex", flexDirection: "column", gap: 10 }}>
                {links.map((link) => (
                  <li key={link.label}>
                    <a
                      href={link.href}
                      style={{ fontSize: 13, color: "rgba(240,244,255,0.5)", textDecoration: "none", transition: "color 0.2s" }}
        ...
        {/* Bottom bar */}
        <div style={{ paddingTop: 24, borderTop: "1px solid rgba(255,255,255,0.05)", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
          ...
          >
            {locale === "es" ? "Configuración de cookies" : locale === "fr" ? "Paramètres des cookies" : locale === "de" ? "Cookie-Einstellungen" : locale === "ca" ? "Configuració de cookies" : "Cookie settings"}
          </a>
        </div>
      </div>
