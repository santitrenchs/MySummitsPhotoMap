...
export function DaisyFlower({ size = 64, className, style }: FlowerProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} style={style}>
      {/* 14 pétalos blancos finos */}
      {Array.from({ length: 14 }, (_, i) => (
        <ellipse
          key={i}
          cx="32" cy="17" rx="2.8" ry="9"
          fill={i % 2 === 0 ? "#FFFFFF" : "#F8FFF8"}
          stroke="#D8ECD8" strokeWidth="0.35"
          transform={`rotate(${i * 25.714} 32 32)`}
        />
      ))}
      {/* Nervio central en cada pétalo */}
      {Array.from({ length: 14 }, (_, i) => (
        <line
          key={i}
          x1="32" y1="23" x2="32" y2="15"
          stroke="#C8E0C8" strokeWidth="0.4" opacity="0.5"
          transform={`rotate(${i * 25.714} 32 32)`}
        />
      ))}
      {/* Centro amarillo */}
...
export function GentianFlower({ size = 64, className, style }: FlowerProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} style={style}>
      ...
      {/* Fondo del tubo con puntos verdes (característica de Gentiana acaulis) */}
...
export function EdelweissFlower({ size = 64, className, style }: FlowerProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} style={style}>
      {/* 8 brácteas estrelladas lanosas */}
      {Array.from({ length: 8 }, (_, i) => (
        <ellipse
          key={i}
          cx="32" cy="14.5" rx="4.8" ry="10"
          fill={i % 2 === 0 ? "#E8EDF6" : "#EDF2FA"}
          stroke="#C5D2E8" strokeWidth="0.3"
          transform={`rotate(${i * 45} 32 32)`}
        />
      ))}
      {/* Nervios lanosos en cada bráctea */}
      {Array.from({ length: 8 }, (_, i) => (
        <line
          key={i}
          x1="32" y1="24" x2="32" y2="13"
          stroke="#B0C4DC" strokeWidth="0.45" opacity="0.45"
          transform={`rotate(${i * 45} 32 32)`}
        />
      ))}
      {/* Pelillos laterales (textura lanosa) */}
      {Array.from({ length: 8 }, (_, i) => (
      ...
      {/* 3 capítulos de flósculos amarillos (agrupados) */}
      ...
      {/* Puntos brillantes en cada capítulo */}
...
export function SaxifrageFlower({ size = 64, className, style }: FlowerProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} style={style}>
      {/* Sépalos verdes (cáliz) */}
      {Array.from({ length: 5 }, (_, i) => (
        <ellipse
          key={i}
          cx="32" cy="16" rx="4" ry="7"
          fill="#3D7A1A"
          transform={`rotate(${36 + i * 72} 32 32)`}
        />
      ))}
      {/* 5 pétalos redondeados blancos con tinte rosado */}
      {Array.from({ length: 5 }, (_, i) => (
        <ellipse
          key={i}
          cx="32" cy="17.5" rx="7" ry="9.5"
          fill={i % 2 === 0 ? "#FFF6F2" : "#FFF1EC"}
          stroke="#FDDCD0" strokeWidth="0.4"
          transform={`rotate(${i * 72} 32 32)`}
        />
      ))}
      {/* Manchas naranjas en cada pétalo (Saxifraga aizoides) */}
      {Array.from({ length: 5 }, (_, i) => (
        <React.Fragment key={i}>
      ...
      {/* 10 estambres con anteras naranjas */}
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} style={style}>
      ...
      {Array.from({ length: 5 }, (_, i) => (
        <ellipse
          key={i}
          cx="32" cy="15.5" rx="5.5" ry="8.5"
          fill="#3D7A1A"
          transform={`rotate(${36 + i * 72} 32 32)`}
        />
      ))}
      {/* 5 pétalos redondeados amarillo-dorados */}
      {Array.from({ length: 5 }, (_, i) => (
        <ellipse
          key={i}
          cx="32" cy="17" rx="7.5" ry="9.8"
          fill={i % 2 === 0 ? "#FDE047" : "#FCD034"}
          stroke="#CA8A04" strokeWidth="0.2"
          transform={`rotate(${i * 72} 32 32)`}
        />
      ))}
      {/* Nervio central en cada pétalo */}
      {Array.from({ length: 5 }, (_, i) => (
        <line
          key={i}
          x1="32" y1="23" x2="32" y2="16"
          stroke="#A16207" strokeWidth="0.5" opacity="0.35"
          transform={`rotate(${i * 72} 32 32)`}
        />
      ))}
      {/* Disco central con estambres */}
      <circle cx="32" cy="32" r="9.5" fill="#B45309" />
      <circle cx="32" cy="32" r="8" fill="#D97706" />
      <circle cx="32" cy="32" r="6.5" fill="#F59E0B" />
      {/* Anillo de estambres con anteras */}
...
export function SnowLotusFlower({ size = 64, className, style }: FlowerProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} style={style}>
      {/* Capa exterior: 6 brácteas papiráceas translúcidas */}
      {Array.from({ length: 6 }, (_, i) => (
        <ellipse
          key={i}
          cx="32" cy="13" rx="6" ry="11"
          fill={i % 2 === 0 ? "#F0ECFF" : "#EAE4FF"}
          stroke="#C4B8E4" strokeWidth="0.5"
          opacity="0.72"
          transform={`rotate(${i * 60} 32 32)`}
        />
      ))}
      {/* Nervios de las brácteas exteriores */}
      {Array.from({ length: 6 }, (_, i) => (
        <line
          key={i}
          x1="32" y1="22" x2="32" y2="13"
          stroke="#A090CC" strokeWidth="0.5" opacity="0.38"
          transform={`rotate(${i * 60} 32 32)`}
        />
      ))}
      {/* Capa interior: 6 brácteas más cortas, desplazadas 30° */}
      {Array.from({ length: 6 }, (_, i) => (
        <ellipse
          key={i}
          cx="32" cy="18.5" rx="4.8" ry="8"
          fill={i % 2 === 0 ? "#DDD4F8" : "#E4DCF8"}
          stroke="#B0A0D8" strokeWidth="0.4"
          opacity="0.88"
          transform={`rotate(${30 + i * 60} 32 32)`}
        />
      ))}
      {/* Racimo central de flósculos morados */}
      <circle cx="32" cy="32" r="10.5" fill="#5B21B6" />
      <circle cx="32" cy="32" r="9" fill="#6D28D9" />
      {/* 9 flósculos tubulares (vista superior) */}
      {[
        [32, 24.5], [36.5, 26], [39.5, 30], [38.5, 35],
        [35, 39], [29, 39], [25.5, 35], [24.5, 30], [27.5, 26],
      ].map(([cx, cy], i) => (
        <React.Fragment key={i}>
