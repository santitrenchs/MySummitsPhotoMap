/**
 * sync-osm-peaks-france.ts
 *
 * Safe OSM sync for France peaks. Four rules applied in order:
 *
 *   1. FR peaks WITH ascents + osmId  → UPDATE fields from OSM in place (never delete)
 *   2. FR peaks WITH ascents, no osmId → fuzzy-match by name+altitude+proximity
 *                                         → if match: UPDATE osmId + fields in place
 *                                         → if no match: keep as-is, log warning
 *   3. FR peaks WITHOUT ascents        → DELETE (safe, no user data lost)
 *   4. OSM nodes not yet in DB         → INSERT as new peaks
 *
 * Input:  /tmp/france-peaks.ndjson  (produced by fetch-osm-peaks-france.ts)
 * Run:
 *   DATABASE_URL="..." npx tsx scripts/sync-osm-peaks-france.ts [--dry-run]
 *
...
const NDJSON    = "/tmp/france-peaks.ndjson";
const BATCH     = 500;
...
async function main() {
  ...
  // Pre-load ALL osmIds already in DB (any country) to avoid inserting peaks
  // that already exist under a different country (e.g. border peaks already as ES)
  ...
  // ── Step 3: DELETE FR peaks without ascents ───────────────────────────────────
  console.log(`Step 3 — Deleting ${withoutAscents.length} FR peaks without ascents…`);
