/**
 * sync-osm-peaks-italy.ts
 *
 * Safe OSM sync for Italian peaks (all regions outside the Alps sync coverage).
 * Same 4-step logic as other sync-osm-peaks-*.ts scripts.
 *
 * Input:  /tmp/italy-peaks.ndjson  (produced by fetch-osm-peaks-italy.ts)
 * Run:
 *   DATABASE_URL="..." npx tsx scripts/sync-osm-peaks-italy.ts [--dry-run]
 */
...
const NDJSON  = "/tmp/italy-peaks.ndjson";
const BATCH   = 500;
...
async function main() {
  ...
  // Fetch existing IT peaks from DB
  ...
  console.log(`IT peaks in DB:        ${dbPeaks.length.toLocaleString()}`);
  ...
  // ── Step 1 ───────────────────────────────────────────────────────────────────
  console.log(`Step 1 — Updating ${withOsmId.length} IT peaks with ascents + osmId…`);
  ...
  // ── Step 2 ───────────────────────────────────────────────────────────────────
  console.log(`Step 2 — Fuzzy-matching ${withoutOsmId.length} IT peaks with ascents but no osmId…`);
  ...
  // ── Step 3 ───────────────────────────────────────────────────────────────────
  console.log(`Step 3 — Deleting ${withoutAscents.length} IT peaks without ascents…`);
  ...
  console.log(`Step 4 — Inserting ${toInsert.length.toLocaleString()} new OSM peaks (country="IT")…`);
  let step4Inserted = 0;
