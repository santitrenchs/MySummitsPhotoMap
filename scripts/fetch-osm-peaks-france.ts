/**
 * fetch-osm-peaks-france.ts
 *
 * Queries the Overpass API for all natural=peak nodes in France
 * that have both a name AND an ele (elevation) tag.
 * Includes mainland + Corsica + overseas territories (Guadeloupe, Martinique,
 * Guyane, La Réunion, Mayotte).
 *
 * Run with:
 *   npx tsx scripts/fetch-osm-peaks-france.ts
 *
...
interface PeakRow {
  ...
  mountainRange: string | null; // tag: mountain_range or region → Peak.mountainRange
  country: string;         // always "FR"            → Peak.country
  ...
  _osm: {
    ...
    wikidata?: string;
    nameFr?: string;
    nameOc?: string;       // Occitan
    nameBreton?: string;   // Breton
...
// ── Overpass query ────────────────────────────────────────────────────────────
//
// Fetches natural=peak nodes inside the area of France (ISO3166-1=FR).
// Using area-based query covers mainland + Corsica + overseas automatically.
...
(
  node["natural"="peak"]["name"]["ele"](area.france);
);
out body;
`.trim();
...
async function main() {
  console.error("Querying Overpass API for France peaks (name + ele)…");
