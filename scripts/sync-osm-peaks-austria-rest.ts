/**
 * sync-osm-peaks-austria-rest.ts
 *
 * Safe OSM sync for remaining Austrian peaks (East + North, outside Alps coverage).
 * Same 4-step logic as other sync-osm-peaks-*.ts scripts.
 *
 * Input:  /tmp/austria-rest-peaks.ndjson  (produced by fetch-osm-peaks-austria-rest.ts)
 * Run:
 *   DATABASE_URL="..." npx tsx scripts/sync-osm-peaks-austria-rest.ts [--dry-run]
 */
...
const NDJSON  = "/tmp/austria-rest-peaks.ndjson";
const BATCH   = 500;
...
async function main() {
  ...
  // Fetch existing AT peaks from DB
  ...
  console.log(`AT peaks in DB:        ${dbPeaks.length.toLocaleString()}`);
  ...
  // ── Step 1 ───────────────────────────────────────────────────────────────────
  console.log(`Step 1 — Updating ${withOsmId.length} AT peaks with ascents + osmId…`);
  ...
  // ── Step 2 ───────────────────────────────────────────────────────────────────
  console.log(`Step 2 — Fuzzy-matching ${withoutOsmId.length} AT peaks with ascents but no osmId…`);
  ...
  // ── Step 3 ───────────────────────────────────────────────────────────────────
  console.log(`Step 3 — Deleting ${withoutAscents.length} AT peaks without ascents…`);
  ...
  console.log(`Step 4 — Inserting ${toInsert.length.toLocaleString()} new OSM peaks (country="AT")…`);
  let step4Inserted = 0;
