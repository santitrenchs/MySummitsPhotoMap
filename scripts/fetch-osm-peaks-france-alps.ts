/**
 * fetch-osm-peaks-france-alps.ts
 *
 * Fetches French Alps peaks NOT covered by previous syncs:
 *   - Alps sync:        46.5–48.5N, 9.5–15.0E  (too far east, misses western Alps)
 *   - Pyrenees FR:      42.3–43.7N, -2.0–3.5E
 *   - france-rest:      Massif Central, Jura, Vosges, Nord, Corsica
 *
 * Missing: French Alps (western, south of 46.5N):
 *   1. Alpes du Nord    — Mont Blanc, Chamonix, Belledonne, Vercors (45.0–46.5N, 5.5–9.5E)
 *   2. Alpes du Sud     — Écrins, Queyras, Mercantour (43.5–45.5N, 5.5–7.5E)
 *
 * Run:
 *   npx tsx scripts/fetch-osm-peaks-france-alps.ts
 *
 * Output: /tmp/france-alps-peaks.ndjson
 */
...
const BBOXES: [number, number, number, number, string][] = [
  [45.0, 5.5, 46.5, 9.5, "Alpes du Nord (Mont Blanc, Chamonix, Belledonne, Vercors)"],
  [43.5, 5.5, 45.5, 7.5, "Alpes du Sud (Écrins, Queyras, Mercantour)"],
];
...
async function fetchBbox(
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    for (const mirror of OVERPASS_MIRRORS) {
      try {
        ...
        if (res.status === 429 || res.status === 504) {
          console.error(`    ${res.status} on ${mirror} — waiting 30s…`);
    ...
    if (attempt < 3) {
      console.error(`  All mirrors failed (attempt ${attempt}/3) — waiting 60s before retry…`);
...
async function main() {
  console.error("Fetching French Alps peaks (2 bboxes)…\n");
