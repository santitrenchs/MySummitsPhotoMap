...
- **Email**: Resend (`resend` npm package), domain `mail.peakadex.com` (configured via Resend + Cloudflare integration), region `eu-west-1`. From address: **`Peakadex <hola@mail.peakadex.com>`** — this is the default in `lib/email.ts` and the `RESEND_FROM` set on Railway. ⚠️ It must be a sender the Resend domain has verified; an unverified one fails exactly like a bad API key, silently (see the Resend gotcha below). This line used to say `noreply@`, which is wrong and has already cost one debugging session. API key in env var `RESEND_API_KEY`, from address in `RESEND_FROM`. Email logic in `lib/email.ts`.
  - ⚠️ **The `RESEND_API_KEY` in `.env.local` is revoked** (verified 2026-09-24): it returns `401 API key is invalid`. So **no email leaves a local dev machine** — password reset, friend invites, cordada invites, tag notifications. Production uses a different, working key. To recover a valid one, read it from Railway (`railway variables --json`); Resend never shows an existing key again, it can only mint a new one.
...
- **Resend returns errors silently**: `resend.emails.send()` does not throw on API errors — it returns `{ data, error }`. Always destructure and check: `const { data, error } = await resend.emails.send(...); if (error) throw new Error(...)`. Otherwise failed sends appear as successes with no logs. The same applies to the raw REST API (used by the tiles Worker, which has no npm package): it answers **HTTP 200 with an error object in the body**, so check the body, never just the status. The two things that fail this way are a **revoked key** and an **unverified `from` address**, and they look identical from the outside — both surface as `{"statusCode":401,"name":"validation_error"}` only once you read the body.
...
**What IS cached is the peak list, which is a different thing (2026-09-16).** The curated list is global and changes only when an admin edits the challenge, while the detail was the slowest endpoint in the app. `getChallengePeaks(challengeId)` in `challenge.service.ts` wraps that query in `unstable_cache` (same pattern as `getLandingStats`), keyed by challenge id — the locale is **not** part of the key because `peakDisplayName()` does not depend on it — and tagged `challenge-peaks:{id}`. Invalidation is by tag: `revalidateTag(peaksTag(id), "max")` from `updateChallenge` **only when `peakIds` changed** and from `deleteChallenge`; the 1h `revalidate` is a backstop, not the mechanism. Next 16 requires the second argument.

⚠️ `unstable_cache` needs Next's incremental cache: it is a no-op-with-invariant-error outside a request, so unit tests mock `next/cache` to a passthrough, and **in `next dev` it does not persist between requests** — measure caching changes against a production build or you will conclude it does nothing.
...
**N+1 traps to avoid:** use `_count` for the peak count (never `include: { peaks: true }`); the detail is a handful of queries + in-memory merge, never one query per peak; reuse `profile.service.ts`'s `firstPhotoUrl` query for row thumbnails; **do not add a participant count** ("N people in this challenge") — it would force a cached counter back in.

**The detail's query shape is deliberate (2026-09-16).** It reads the challenge, the membership and the cached peak list **in parallel**, then the user's ascents on those peaks. Membership is its own `findUnique` on the composite PK rather than a nested `participants` include, so it rides along in the same wave instead of costing a round trip. Do **not** go back to `challenge.findUnique({ include: { peaks: { include: { peak } } }, participants } })`: Prisma walked `challenges → challenge_peaks → peaks → challenge_participants` one round trip at a time, and this endpoint is latency-bound, not data-bound (39 KB of payload, indexes all in place, every query costing about what a ping costs). Measured on a production build against staging with a 239-peak challenge: **~1290 ms → ~620 ms**, from 6-7 waves to 4.
...
- **"Mis retos" = one card per challenge**: the challenge's **60px patch** + name + percentage + 3px progress bar + subtitle, in the same shell as the detail's peak rows (white, `--radius-lg`, `1px solid rgba(13,37,56,0.06)`, the two-layer shadow), stacked with `gap: 10`. ⚠️ It first shipped as flat rows on a single white surface with inset dividers, copying Amigos/Cordadas — with two challenges they read as one glued block. Retos are few by definition, so separating them costs nothing; do not fold them back into a shared surface.
...
- **The patch renders the same in both places** — 60px, `object-fit: contain`, no circular clip, soft drop-shadow. `cover` inside a circle shaved the artwork's own ring off. Challenges *without* a patch keep their circular placeholder. See `DESIGN.md → "Retos — Web"`.

### UI — Reto detail (pushed screen, redesigned 2026-09-16)

The screen is **two halves, not one list**: captured peaks are a photo wall, pending
peaks are text lines. The split carries the state — **a photo means conquered, a text
line means still out there** — which is why the done rows no longer need a green tick.
Before, every peak was an 84px card, so 123 pending ones were 123 repetitions of the
navy missing-photo placeholder.

- Breadcrumb `‹ RETOS` (see "Going back from a detail screen") + the header below it.
- **Header**: the challenge's **76px patch** (`coverUrl`, already returned by `getChallengeDetail`) takes the space that was empty to the left of the figures, then name + description (2 lines) + progress bar with the `%` to its right + the figures line `27 / 150 · 123 pendientes` with `MÁS ALTA 3143 m` pushed right. No patch → the header renders exactly as before.
- **Width stays at Bitácora's 640px.** A wider column stretched the shared `BitacoraTabs` strip (its tabs are `flex: 1`) and the detail stopped matching the other tabs. Anything wider has to solve the strip first.
- **"Tu colección"** (`challenges_sectionCollection`): 4:5 mosaic, 3 columns on a phone and 4 from 640px, rarity badge top-left (the same 20px white capsule as the Fotos grid), name + altitude over the gradient. The ascent date joins the caption **only ≥640px** — at 3 columns on 375px it wrapped out of the gradient. Tapping goes to that peak's cards.
- **"Pendientes"** (`challenges_filterPending`): one white sheet, **40px rows** split by `1px #F1F5F8` — rarity flower · name · range · altitude (mono, right-aligned). **One column at every width.** The range shows only ≥640px and never falls back to the country code ("ES" on every row of a Pyrenean list says nothing).
- The whole pending row opens the create-ascent modal with the peak preselected (same event the map panel dispatches); it tints `#F7FAFC` on pointer and has a visible focus outline. ⚠️ **No "Registrar" button on hover**: it reserved ~120px and left the altitude floating mid-row; not reserving it made the text jump when it appeared.
- **The full peak list, always** — never a "+N más". Filters hide, they never truncate. Status "Hechas"/"Pendientes" simply leaves one half on screen.
- `Buscar cima…` + `Filtros` + the Atlas CTA (`PeakFiltersBar` family); Filtros opens a sheet with **Estado: Todos / Hechos / Pendientes**, **Ordenar por**, and a "Ver picos" CTA.
...
- **Percentage, right of the bar** (mono, `#2F7A5F`), on both the row and the detail — `lib/progress-pct.ts` → `progressPct()`. It never rounds to a figure that contradicts the fraction beside it: 521/522 is 99%, not 100%, and 1/522 is 1%, not 0%. In the **row** the percentage takes the prominent right-hand slot and the `2/8` fraction moves down into the subtitle (`2/8 · Quedan 6 cimas`).
...
- **A long challenge is 500 rows and a wall of photos**: tiles carry `loading="lazy"` + `decoding="async"`, and tiles and pending rows carry `content-visibility: auto` with `contain-intrinsic-size` so off-screen items cost no layout or paint.
- Gone with the redesign: the `PeakRowCard` clone and the keys `challenges_lastLabel`, `challenges_noAscent` and `challenges_detailEyebrow` (the date now sits in the tile caption, the absence of a photo is what says "pending", and the challenge name is the title).

Full visual spec in `DESIGN.md → "Retos — Web (listado y detalle)"`.
